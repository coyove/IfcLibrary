...
/*
 * UnsafeDoubleParser is from FloatingDecimal, but refactored for better char buffer reading.
 * It's single-threaded only.
 */
/*
...
public class UnsafeDoubleParser {
    private static boolean isNegative;
    private static int decExponent;
    private static char digits[];
    private static int nDigits;
    private static int bigIntExp;
    private static int bigIntNBits;
     ...
     *
     */
    private static FDBigInt doubleToBigInt(double dval) {
    ...
    private static boolean __isNegative = false;
    private static boolean __signSeen = false;
    private static char[] __digits = new char[256];
    private static int __nDigits = 0;
    private static boolean decSeen = false;
    private static boolean expSeen = false;
    private static int decPt = 0;
    private static int decExp = 0;
    private static int nLeadZero = 0;
    private static int nTrailZero = 0;
    private static int potentialInteger = 0;
    private static int expSign = 1;
    private static int expVal = 0;
    private static int reallyBig = Integer.MAX_VALUE / 10;
    private static boolean expOverflow = false;

    public static double parse(String in) throws NumberFormatException {
        int decExp;
        char c;
        int l = in.length();
...
//                    break parseNumber; // certainly bad
        ...
        return doubleValue(__isNegative, decExp, digits, nDigits);
    }
        ...
        switch (c) {
            ...
            case '9':
                if (expSeen) {
                    ...
                    while (nTrailZero > 0) {
                        __digits[__nDigits++] = '0';
                        nTrailZero -= 1;
                    }
                    __digits[__nDigits++] = c;
                    potentialInteger = potentialInteger * 10 + (int) c - (int) '0';
                }
                break; // out of switch.
            case '.':
                if (decSeen) {
    ...
    private static void seal() {
         ...
         */
        if (expSeen) {
            int expLimit = bigDecimalExponent + __nDigits + nTrailZero;
            if (expOverflow || (expVal > expLimit)) {
                //
                // The intent here is to end up with
                // infinity or zero, as appropriate.
...
//                break parseNumber; // certainly bad
    ...
    /*
     * Take a UnsafeDoubleParser, which we presumably just scanned in,
    ...
    private static strictfp double doubleValue(boolean negSign, int decExponent, char[] digits, int n) {
        isNegative = negSign;
        nDigits = n;
